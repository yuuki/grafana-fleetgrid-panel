...
The automated end-to-end tests run against Grafana's built-in TestData data source. Verification against a live Prometheus / VictoriaMetrics instance — including the instant-query drilldown re-query — is still outstanding and recommended before production use.
...
Cells are built from the union of all queries, so a node present in only one query still appears; cells with no sample for the displayed query use the missing color. To view all metrics at once, enable split mode under **Display**.
